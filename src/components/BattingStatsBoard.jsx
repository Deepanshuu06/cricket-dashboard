...
// Notice the new 'onPlayerClick' prop added here
const BattingStatsBoard = ({ onBatsmanClick, onRunClick, onSrClick, onPlayerClick }) => {
  const data = useScoreStore((state) => state.liveData);

  const [overlay, setOverlay] = useState(null);
  const [strikeChangeOverlay, setStrikeChangeOverlay] = useState(false);
  const [bowlerChangeOverlay, setBowlerChangeOverlay] = useState(false);
  ...
  // 3. Bowler Change Overlay Effect
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="running-border-container shadow-[0_4px_15px_rgba(0,0,0,0.5)]">
        <div className="running-border-inner">
          <div className="h-[70px] flex items-center bg-gradient-to-b from-orange-500 to-orange-700 text-white font-black px-6 text-[45px] tracking-wider uppercase border-b-[4px] border-black shadow-[inset_0_4px_8px_rgba(255,255,255,0.3)] relative overflow-hidden">
            ...
            {/* RESTORED: This opens the Match Summary */}
            <div onClick={onBatsmanClick} className="w-[42%] z-10 cursor-pointer hover:text-yellow-300">BATSMAN</div>
            ...
            <div onClick={onSrClick} className="w-[18%] text-right cursor-pointer z-10">S.R</div>
          </div>
          ...
          {batters.map((batter, idx) => (
            ...
            >
              ...
              {/* NEW PROP APPLIED HERE: This opens the new Player Stats Modal */}
              <div 
                onClick={() => onPlayerClick && onPlayerClick(batter)}
                className="w-[42%] flex items-center gap-3 text-[65px] tracking-tight whitespace-nowrap leading-none z-10 cursor-pointer hover:text-yellow-400 transition-colors"
              >
                {batter.onStrike ? (
                  <motion.div
                ...
                {batter.onStrike && (
                  <motion.div animate={{ rotate: [-45, -25, -45] }} transition={{ repeat: Infinity, duration: 1.5 }}>
                    <GiCricketBat className="text-yellow-400 drop-shadow-md ml-2" size={45} />
                  </motion.div>
                )}
              </div>

              <div className="w-[10%] text-center text-[65px] font-black leading-none z-10">{String(batter.runs || 0)}</div>
              <div className="w-[10%] text-center text-[55px] leading-none z-10">({String(batter.balls || 0)})</div>
              <div className="w-[10%] text-center text-[55px] leading-none z-10">{String(batter.fours || 0)}</div>
              <div className="w-[10%] text-center text-[55px] leading-none z-10">{String(batter.sixes || 0)}</div>
              <div className="w-[18%] text-right text-[60px] font-black tracking-tighter leading-none z-10 text-yellow-300">{String(batter.strikeRate || 0)}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="h-[60px] flex items-center bg-gradient-to-b from-orange-500 to-orange-700 text-white font-black px-4 text-[35px] tracking-wider uppercase border-b-[4px] border-black">
        <div className="w-[70%] flex items-center">
          <div className="w-[35%] flex items-center gap-3">
            <div className="bg-white/20 p-1.5 rounded-full"><MdSportsCricket size={24} /></div>
            BOWLER
          </div>
        ...
        <div className="w-[30%] flex items-center justify-center gap-3 relative overflow-hidden h-[60px]">
          <AnimatePresence mode="wait">
            <motion.div key={showLastWicket ? "wkt" : "ptn"} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} transition={{ duration: 0.3 }} className="absolute flex items-center gap-3">
              <div className="bg-white/20 p-1.5 rounded-full"><FaUserShield size={22} /></div>
      ...
      <div className="relative h-[95px] flex items-center bg-gradient-to-b from-blue-700 to-blue-900 text-white font-bold px-4 overflow-hidden">
        <AnimatePresence>
          {bowlerChangeOverlay && (
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "tween", duration: 0.4 }} className="absolute inset-0 z-20 flex items-center justify-center bg-gradient-to-r from-purple-700 to-purple-500 text-white">
              <span className="text-[55px] font-black tracking-widest uppercase drop-shadow-lg">BOWLER CHANGE</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="w-[70%] flex items-center z-10">
          ...
          {/* NEW PROP APPLIED HERE: This opens the new Player Stats Modal */}
          <div 
            onClick={() => onPlayerClick && onPlayerClick(bowler)}
            className="w-[35%] text-[45px] leading-none tracking-tight whitespace-nowrap cursor-pointer hover:text-yellow-400 transition-colors"
          >
            {String(bowler?.name || "")}
          </div>
          
          <div className="w-[15%] text-center text-[55px] font-black text-blue-200 leading-none">{String(bowler?.overs || 0)}</div>
          <div className="w-[15%] text-center text-[55px] font-black leading-none">{String(bowler?.figures?.split("-")?.[1] || "0")}</div>
          <div className="w-[15%] text-center text-[55px] font-black leading-none text-red-400">{String(bowler?.figures?.split("-")?.[0] || "0")}</div>
          <div className="w-[20%] text-center text-[55px] font-black text-blue-200 leading-none">{String(bowler?.econ || 0)}</div>
        </div>
        ...
        <div className="w-[30%] flex items-center justify-center relative h-[95px] overflow-hidden px-2 z-10">
          <AnimatePresence mode="wait">
            <motion.div key={showLastWicket ? "wkt-val" : "ptn-val"} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} transition={{ duration: 0.3 }} className={`absolute w-full flex flex-col items-center justify-center font-black tracking-tighter leading-none drop-shadow-md ${showLastWicket ? "text-red-400 text-[38px]" : "text-orange-400 text-[65px]"}`}>
              {showLastWicket ? (
                <>
                  <div className="truncate w-full text-center text-yellow-300 ">{typeof lastWicket === "object" ? lastWicket.name || "" : lastWicket || ""}</div>
                  <div className="text-[32px] opacity-90 mt-1 text-white">{typeof lastWicket === "object" ? `(${lastWicket.score || 0})` : ""}</div>
                </>
              ) : displayPartnership}
            </motion.div>
          </AnimatePresence>
      ...
      {yet_to_bat && yet_to_bat.length > 0 && (
        <div className="h-[60px] flex items-center bg-gradient-to-b from-gray-800 to-gray-950 text-white font-bold border-t-[4px] border-black overflow-hidden relative animate-marquee-container">
          <div className="absolute left-0 top-0 bottom-0 z-10 bg-gradient-to-r from-gray-900 via-gray-900 to-transparent pr-12 pl-6 flex items-center border-r-[2px] border-gray-700/50">
            <span className="text-[32px] text-gray-300 font-black tracking-tighter uppercase whitespace-nowrap">Yet to Bat:</span>
          </div>
          <div className="w-full overflow-hidden">
            <div className="animate-marquee flex w-max items-center gap-10 text-[32px] font-bold tracking-tight text-gray-100">
              {[...yet_to_bat, ...yet_to_bat].map((player, index) => (
                <span key={`${player.name}-${index}`} className="flex items-center gap-2 whitespace-nowrap">
                  <span className="inline-block w-[8px] h-[8px] rounded-full bg-orange-500" />
                  <span>{player.name}</span>
                  <span className="font-semibold text-yellow-300">({player.average ?? "-"})</span>
                </span>
              ))}
            </div>
          </div>
